...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function assinarAtualizacoesPrestadores() {
        ...
        canalPrestadores = supabaseClient
            .channel("prestadores-disponiveis")
    ...
    function renderizarPrestadores(categoria = "todos") {
        ...
        if (prestadores.length === 0) {
            prestadoresGrid.innerHTML = `
                <div class="empty-pros-msg" style="grid-column: 1 / -1;">
                    <span>🔍</span>
                    <p>Nenhum profissional encontrado ${categoria !== 'todos' ? 'para a categoria selecionada' : 'cadastrado no banco'} no momento.</p>
                </div>
            `;
            return;
        }
        ...
        prestadores.forEach(p => {
            ...
            const notaFormatada = p.avaliacao == null ? null : p.avaliacao.toFixed(1);
                ...
                <div class="pro-card-header">
                    ...
                    <div class="pro-main-info">
                        ...
                        <div class="pro-rating-row">
                            ${notaFormatada === null ? '<small class="rating-count">Sem avaliações</small>' : `<span class="stars-gold">${gerarEstrelasHtml(p.avaliacao)}</span><strong class="rating-num">${notaFormatada}</strong>`}
                            <small class="rating-count">(${totalAval} avaliações)</small>
                        </div>
                    </div>
                </div>

                ${p.descricao ? `<p class="pro-short-bio">${p.descricao.substring(0, 95)}${p.descricao.length > 95 ? '...' : ''}</p>` : ''}
    ...
    function abrirPerfilPrestador(prestadorId) {
        ...
        if (bioEl) bioEl.textContent = p.descricao || "Descrição não informada.";
