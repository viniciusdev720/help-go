...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (statusToggleBtn) {
        statusToggleBtn.addEventListener("click", async () => {
            ...
            if (prestadorAtual.id) {
                await PrestadoresService.atualizarStatus(prestadorAtual.id, isOnline, isOnline ? "disponivel" : "indisponivel");
            }
                ...
                <div class="request-card-item" id="req-card-${p.id}">
                    <div class="card-top">
                        <div class="service-pill ${pillClass}">
                            ${escaparHtml(p.servico)}
                        </div>
                        ${urgencyBadge}
                    </div>

                    <h3 class="request-title">${escaparHtml(p.descricao.length > 55 ? p.descricao.substring(0, 55) + '...' : p.descricao)}</h3>
                    <p class="request-desc">${escaparHtml(p.descricao)}</p>
                    ...
                    <div class="request-meta">
                        ...
                        <div class="meta-item">
                            ...
                            <strong>${escaparHtml(p.agendamento ? `Solicitado: ${p.agendamento}` : 'Agendamento não informado')}</strong>
                        </div>
                        <div class="meta-item">
                            
                            <strong class="budget-tag">${escaparHtml(p.orcamento)}</strong>
                        </div>
                    </div>
        ...
        activeServicesList.innerHTML = emAndamento.map(p => `
            <div class="active-item" id="active-req-${p.id}">
                <div class="active-item-info">
                    <strong>${escaparHtml(p.servico)} - ${escaparHtml(p.descricao.substring(0, 45))}...</strong>
    ...
    window.abrirDetalhesStore = function(pedidoId) {
        ...
        document.getElementById("modalDescription").textContent = p.descricao +
            (p.agendamento ? `\nDia e horário solicitados: ${p.agendamento}` : "");
    ...
    function assinarAtualizacoesSolicitacoes() {
        if (!supabaseClient || !prestadorAtual.id || canalSolicitacoes || typeof supabaseClient.channel !== "function") return;
        canalSolicitacoes = supabaseClient
            .channel(`solicitacoes-prestador-${prestadorAtual.id}`)
            .on("postgres_changes", { event: "*", schema: "public", table: "solicitacoes" }, async () => {
                await renderizarChamados();
                await renderizarEmAndamento();
            })
            .subscribe();
