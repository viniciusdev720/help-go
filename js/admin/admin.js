...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function ativarSecao(nome) {
        ...
        const [titulo, breadcrumb] = labels[nome] || ["Visão geral", "Resumo"];
    ...
    async function carregarClientes(completo = false) {
        ...
        try {
            const separados = await AuthService.perfisSeparadosAtivos();
            let consulta = supabaseClient.from(separados ? "clientes" : "usuarios")
                .select("id, nome, email, created_at");
            if (!separados) consulta = consulta.eq("tipo", "cliente");
            ...
            const htmlItens = clientes.length === 0
                ? `<div class="empty-admin-msg">Nenhum cliente cadastrado.</div>`
            ...
            const msg = `<div class="empty-admin-msg">Erro ao carregar clientes.</div>`;
    ...
    async function carregarPrestadores(completo = false) {
        ...
        try {
            let prestadores;
            if (await AuthService.perfisSeparadosAtivos()) {
                const [perfis, disponibilidade] = await Promise.all([
                    supabaseClient.from("prestador").select("id, nome, email").order("nome"),
                    supabaseClient.from("prestadores").select("id, ativo")
                ]);
                if (perfis.error) throw perfis.error;
                if (disponibilidade.error) throw disponibilidade.error;
                const statusPorId = new Map((disponibilidade.data || []).map(p => [p.id, p.ativo]));
                prestadores = (perfis.data || []).map(p => ({
                    id: p.id, ativo: statusPorId.get(p.id) === true, usuarios: p
                }));
            } else {
                const { data, error } = await supabaseClient.from("prestadores")
                    .select("id, ativo, usuarios (nome, email)")
            ...
            const htmlItens = prestadores.length === 0
                ? `<div class="empty-admin-msg">Nenhum prestador cadastrado.</div>`
            ...
            const msg = `<div class="empty-admin-msg">Erro ao carregar prestadores.</div>`;
            if (containerRapido)   containerRapido.innerHTML   = msg;
            if (containerCompleto) containerCompleto.innerHTML = msg;
        }
    }
    ...
    async function carregarSolicitacoes(completo = false) {
        ...
        try {
            const { data, error } = await supabaseClient
                .from("solicitacoes")
                .select("id, descricao, status, criado_em, servicos(nome)")
            ...
            const htmlItens = solicitacoes.length === 0
                ? `<div class="empty-admin-msg">Nenhuma solicitação registrada.</div>`
                    ...
                    return `
                        <div class="request-list-item">
                            <div class="request-info">
                                <div class="request-title">${s.servicos?.nome || "Serviço"}</div>
            ...
            const msg = `<div class="empty-admin-msg">Erro ao carregar solicitações.</div>`;
            if (containerRapido)   containerRapido.innerHTML   = msg;
            if (containerCompleto) containerCompleto.innerHTML = msg;
        }
    }
        ...
        try {
            ...
            if (listaCompleta) {
                listaCompleta.innerHTML = servicos.length === 0
                    ? `<div class="empty-admin-msg">Nenhum serviço cadastrado.</div>`
                    : servicos.map(s => `
                        <div class="user-list-item">
                            <div class="user-list-info">
